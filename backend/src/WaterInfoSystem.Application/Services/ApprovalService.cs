...
public class ApprovalService : IApprovalService
{
    ...
    public ApprovalService(IApprovalRepository approvalRepository, IUnitOfWork unitOfWork, IAiServiceClient aiClient)
    {
    ...
    {
        var (items, total) = await _approvalRepository.SearchAsync(query.Keyword, query.Status, query.Page, query.PageSize, cancellationToken);
    ...
    public async Task<ApprovalDetailDto> CreateAsync(ApprovalCreateDto request, Guid userId, CancellationToken cancellationToken)
    {
        ...
        await _approvalRepository.AddAsync(entity, cancellationToken);
    ...
    {
        var entity = await GetRequiredEntityAsync(id, cancellationToken);
        await _approvalRepository.DeleteAsync(entity, cancellationToken);
        await _unitOfWork.SaveChangesAsync(cancellationToken);
    }
    ...
    {
        var entity = await _approvalRepository.GetByIdAsync(id, cancellationToken);
        if (entity is null)
